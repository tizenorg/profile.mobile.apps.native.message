...
ConvList::ConvList(Evas_Object *parent, App &app)
    ...
    , m_pListner(nullptr)
    , m_App(app)
    , m_OwnerThumbId(m_App.getThumbnailMaker().makeDefaultThumb(ThumbnailMaker::OwnerThumb))
    , m_RecipThumbId(m_App.getThumbnailMaker().makeDefaultThumb(ThumbnailMaker::SingleThumb))
    , m_SearchWord()
{
    create(parent);
}
...
void ConvList::setThreadId(ThreadId id, const std::string &searchWord)
{
    if(m_ThreadId != id || m_SearchWord != searchWord)
    {
        ...
        if(addressList)
        {
            ...
            if(countContact > 1)
            {
                m_RecipThumbId = m_App.getThumbnailMaker().makeDefaultThumb(ThumbnailMaker::GroupThumb);
            }
            else if(countContact == 1)
            {
                m_RecipThumbId = m_App.getThumbnailMaker().makeThumbFromAddress(addressList->at(0));
            }
            else
            {
