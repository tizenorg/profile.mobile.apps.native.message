    ...
    , m_pListener(nullptr)
{
    ...
    MbeRecipients *pMbe = new MbeRecipients(*this, m_App);
    pMbe->addSmartCb("item,added", SMART_CALLBACK(ConvRecipientsPanel, onMbeChanged), this);
    pMbe->addSmartCb("item,deleted", SMART_CALLBACK(ConvRecipientsPanel, onMbeChanged), this);
    pMbe->setListener(this);
    pMbe->show();
    setMbe(pMbe);
}
...
{
    dynamic_cast<MbeRecipients*>(m_pMbe)->update(threadId);
}
...
{
    MbeRecipients::AppendItemStatus appendStatus =
            dynamic_cast<MbeRecipients*>(m_pMbe)->appendItem(address, dispName, addressType);
...
{
    MbeRecipients::AppendItemStatus appendStatus =
            dynamic_cast<MbeRecipients*>(m_pMbe)->appendItem(address, addressType);
