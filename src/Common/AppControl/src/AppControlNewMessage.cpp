...
void AppControlNewMessage::parce(app_control_h handle)
{
    char *key = nullptr;
    app_control_get_extra_data(handle, "type", &key);
    if(!key)
        return;
    ...
    else if(keyType == "new_msg")
        viewMessage(handle);
    else
        MSG_LOG("Unknown type!");

    char *msgIdStr = nullptr;
    app_control_get_extra_data(handle, "msgId", &msgIdStr);
    if(!msgIdStr)
        return;
